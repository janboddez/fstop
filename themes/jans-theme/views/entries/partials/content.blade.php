<article class="h-entry {{ $entry->type }}{{ ! empty($entry->thumbnail) ? ' has-thumbnail' : '' }}">
    ...
    @if (! in_array($entry->type, ['article', 'page'], true))
        ...
        <footer class="entry-footer">
            <div class="entry-meta">
                <a class="u-url" href="{{ route(Str::plural($entry->type) . '.show', $entry->slug) }}" rel="bookmark"><time class="dt-published" datetime="{{ $entry->published ? $entry->published->format('c') : $entry->created_at->format('c') }}">{{ $entry->published ? $entry->published->format('M j, Y') : $entry->created_at->format('M j, Y') }}</time></a>
