...
class ProcessWebmention implements ShouldQueue
{
    ...
    {
        // Fetch HTML.
        list($html, $status) = Cache::remember("html:{$this->webmention->source}", 60 * 60, function () {
            $response = Http::withHeaders([
                    'User-Agent' => Eventy::filter(
                        'webmention:user_agent',
                        'F-Stop/' . config('app.version') . '; ' . url('/'),
                        $this->webmention->source
                    ),
                ])
                ->get($this->webmention->source);

            if (! $response->successful()) {
                Log::error("[Webmention] Failed to retrieve the page at {$this->webmention->source}");
        ...
        if (in_array($status, [404, 410], true) || strpos($html, $this->webmention->target) === false) {
            // The source page no longer exists or simply does not mention our "target." Delete previous mentions, if
            // any.
            $deleted = Comment::whereHas('meta', function ($query) {
                $query->where('key', 'target')
                    ->where('value', json_encode((array) $this->webmention->target));
            })
            ->whereHas('meta', function ($query) {
                $query->where('key', 'source')
                    ->where('value', json_encode((array) $this->webmention->source));
            })
            ->delete();

            if ($deleted) {
                // phpcs:ignore Generic.Files.LineLength.TooLong
                Log::info("Deleted webmention for source {$this->webmention->source} and target {$this->webmention->target}");

                DB::update(
                    'UPDATE webmentions SET status = ? WHERE id = ?',
                    ['deleted', $this->webmention->id]
                );
            } else {
                DB::update(
                    'UPDATE webmentions SET status = ? WHERE id = ?',
                    ['invalid_source', $this->webmention->id]
                );
            }
        ...
        $entry = url_to_entry($this->webmention->target);

        if (! $entry) {
            // Target URL was removed, or does not accept webmentions.
            DB::update(
                'UPDATE webmentions SET status = ? WHERE id = ?',
                ['invalid_target', $this->webmention->id]
            );
        ...
        // Some default comment data.
        $data = [
            'author' => __('Anonymous'),
            'author_url' => parse_url($this->webmention->source, PHP_URL_SCHEME) . '://' .
                parse_url($this->webmention->source, PHP_URL_HOST),
            'content' => __('&hellip; mentioned this.'),
            'status' => Eventy::filter('comment:set_status', 'pending', $this->webmention->source),
            'type' => 'mention',
            'created_at' => now(),
        ];

        // Parse in any microformats.
        $this->parseMicroformats($data, $html, $this->webmention);
        ...
        if ($comment) {
            Log::debug("[Webmention] Found existing reaction to {$entry->permalink} for {$this->webmention->source}");
            $comment->update($data);
        } else {
            Log::debug("[Webmention] Creating new reaction to {$entry->permalink} for {$this->webmention->source}");
        ...
        $comment->meta()->updateOrCreate(
            ['key' => 'source'],
            ['value' => (array) $this->webmention->source]
        );

        $comment->meta()->updateOrCreate(
            ['key' => 'target'],
            ['value' => (array) $this->webmention->target]
        );

        DB::update(
            'UPDATE webmentions SET status = ?, updated_at = NOW() WHERE id = ?',
            [$comment->wasRecentlyCreated ? 'created' : 'updated', $this->webmention->id]
        );
    }
